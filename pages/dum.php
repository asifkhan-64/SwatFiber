...
<div class="container">
    ...
    <p class="lead">Billing calculations based on the connection date and a fixed 30-day cycle.</p>

    <?php

    // --- 1. SAMPLE USER DATA (Updated to use connection_date) ---
     ...
     * * @param array $user User data array (must contain 'connection_date').
    ...
    function calculateBill(array $user, DateTime $today): array {
        ...
        // 3. Total Due 
        // We assume the bill covers the full monthly price for the upcoming cycle
        ...
        <table class="table table-hover table-striped">
            ...
            <tbody>
                <?php foreach ($users as $user): ?>
                    ...
                    // Add a warning class for users expiring soon
                    $row_class = $billing_details['status'] === 'Expired' ? 'table-danger' : '';
                    
                    $days_until_expiry = $billing_details['days_until_expiry'];
                    
                    if ($days_until_expiry <= 5 && $billing_details['status'] === 'Active') {
                        $row_class = 'table-warning'; // Yellow warning if expiring in 5 days or less
                    }
                    ?>
                    <tr class="<?php echo $row_class; ?>">
                        ...
                        <td>
                            ...
                            <?php if ($billing_details['status'] === 'Active'): ?>
                                <span class="badge bg-info text-dark ms-2"><?php echo $days_until_expiry; ?> days left</span>
                            <?php endif; ?>
                        </td>
                        <td>
                        ...
                        <td>
                            <span class="badge bg-<?php echo $billing_details['status'] === 'Active' ? 'success' : 'danger'; ?>">
                                <?php echo $billing_details['status']; ?>
                            </span>
                        </td>
                    </tr>
                <?php endforeach; ?>
    ...
    <div class="alert alert-info mt-4">
        <strong>Note on Billing:</strong> The cycle start date is calculated by finding the most recent date that is an exact multiple of 30 days past the original connection date. The total bill due is the full price for the cycle that starts on the Next Expiry Date.
    </div>
