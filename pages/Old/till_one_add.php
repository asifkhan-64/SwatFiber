...
if (isset($_POST['addTillReport'])) {
    ...
    $checkExisting = mysqli_query($connect, "SELECT * FROM till_one_reports WHERE report_date = '$report_date'");
    if (mysqli_num_rows($checkExisting) > 0) {
        $alreadyAdded = '
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <strong>Error!</strong> Report for this date already added!
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>';
    }else {
        $insertTillReport = mysqli_query($connect, "INSERT INTO till_one_reports (report_date, z_report, daily_sale, card_payment, cash_payment, refund_payment, total_amount, extra) VALUES ('$report_date', '$z_report', '$daily_sale', '$card_payment', '$cash_payment', '$refund_payment', '$total_amount', '$extra')");
        if (!$insertTillReport) {
            $error = '
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <strong>Error!</strong> Not Added! Try again!
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>';
        } else {
            header("LOCATION: till_one_list.php");
        }
    }
...
<div class="page-content-wrapper ">
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                <h5 class="page-title">Generate Till 1 Report</h5>
            </div>
        </div>
        <br>
        <!-- end row -->
                ...
                <div class="card m-b-30">
                    <div class="card-body">
                        <!-- <div class="w-full max-w-4xl bg-white shadow-xl rounded-xl p-6 sm:p-1   md:p-6 lg:p-8 mx-auto"> -->
                            <h3 class="text-1xl font-bold text-blue-600 mb-6 border-b pb-2">Daily Sale & Payment Tracker Till 1 Report</h3>
                            ...
                            <form id="paymentForm" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                ...
                                <div class="space-y-6">
                                    <div class="flex flex-col">
                                        ...
                                        <input type="date" id="date" name="report_date" class="p-3 border border-gray-300 rounded-lg focus:outline-none transition duration-150" required>
                                    </div>
                                    <div class="flex flex-col">
                                        <label for="zReport" class="text-sm font-medium text-gray-700 mb-1">Z Report #</label>
                                        <input type="number" id="zReport" value="0" name="z_report" min="0" class="p-3 border border-gray-300 rounded-lg focus:outline-none transition duration-150" required>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-6">
                                    <div class="flex flex-col">
                                        ...
                                        <input type="number" id="dailySale" name="daily_sale" placeholder="Daily Sale Amount" required value="0" min="0" step="0.01" onkeyup="calculatePayments()" class="p-3 border border-gray-300 rounded-lg focus:outline-none transition duration-150">
                                    </div>
                                    
                                    <div class="flex flex-col">
                                        <label for="cardPayment" class="text-sm font-medium text-gray-700 mb-1">Card Payment</label>
                                        <input type="number" id="cardPayment" name="card_payment" placeholder="Card Payment Amount" required value="0" min="0" step="0.01" onkeyup="calculatePayments()" class="p-3 border border-gray-300 rounded-lg focus:outline-none transition duration-150">
                                    </div>

                                    <div class="flex flex-col">
                                        <label for="cashPayment" class="text-sm font-medium text-gray-700 mb-1">Cash Payment</label>
                                        <input type="number" id="cashPayment" name="cash_payment" placeholder="Cash Payment Amount" required value="0" min="0" step="0.01" onkeyup="calculatePayments()" class="p-3 border border-gray-300 rounded-lg focus:outline-none transition duration-150">
                                    </div>
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <input type="number" id="refundPayment" name="refund_payment" placeholder="Refund Amount" required value="0" min="0" step="0.01" onkeyup="calculatePayments()" class="p-3 border border-gray-300 rounded-lg focus:outline-none transition duration-150">
                                    </div>
                                </div>
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="flex flex-col w-100">
                                        <button type="submit" class="btn btn-primary waves-effect waves-light" name="addTillReport">Add Till Report</button>
                                    </div>
                                    <div class="flex flex-col w-100">
